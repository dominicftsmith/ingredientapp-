...
class InventoryManager:
    ...
    def is_low(self, ingredient_name: str) -> bool:
        return self.get_quantity(ingredient_name) < LOW_INVENTORY_THRESHOLD
...
app = FastAPI(
    ...
    version="1.5.0" # Version Bump: Added GET /inventory endpoint
)
...
def check_stock_logic(item: Dict) -> Dict:
    ...
    for sub in subs_list:
        ...
        if trigger == active_trigger_name:
            ...
            # Note: Negative "We are out of..." message removed as requested.
...
def find_inventory_candidates(query: str, n_results=5) -> List[InventorySearchResult]:
    ...
    for i in range(len(results['ids'][0])):
        ...
        status = "High"
        if qty <= 0: status = "Out"
        elif qty < LOW_INVENTORY_THRESHOLD: status = "Low"
